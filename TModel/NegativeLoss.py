...
import numpy as np
from TUtils.EmbedUtils import AliasMultinomial
...
class SGNSLoss(nn.Module):
    ...
    def __init__(self, word_embeddings, vocabLength, cfs):
        super(SGNSLoss, self).__init__()
        self.criterion = nn.BCEWithLogitsLoss()
        self.vocab_len = vocabLength
        ...
        self.transformed_freq_vec = t.tensor(
            np.array([cfs[i] for i in range(self.vocab_len)]) ** self.BETA
        )
    ...
    def forward(self, center, context):
        ...
        for i in range(self.NUM_SAMPLES):
            samples = self.get_unigram_samples(center)
